...
For example consider a catalogue of products. Each product has details like a laptop has details about its memory and processor a shirt has details about its size and colour and a book has details about its author and book number. If we use a database you have two options, neither of which is good. We can create a table with many columns that are mostly empty or we can use some clever tricks that make it hard to get the information you need quickly. Neither of these options is a solution for a product catalogue.
